...
async function signedSessionCookie(body) {
  ...
  return `__Host-edge_session=${encodeURIComponent(token)}`;
}

async function signedLoginCookie(body) {
  const token = await sign(JSON.stringify(body), config.sessionKey);
  return `__Host-edge_login=${encodeURIComponent(token)}`;
}

describe("session", () => {
  it("uses the project cookie name", () => { expect(SESSION_COOKIE).toBe("__Host-edge_session"); });

  it("session cookie meets __Host- requirements (prefix, Secure, Path=/, no Domain)", async () => {
    expect(SESSION_COOKIE.startsWith("__Host-")).toBe(true);
    const setCookie = await mintSessionCookie({ sub: "user-123", groups: [] }, config);
  ...
  it("mints then reads a valid session (edge keeps email + name)", async () => {
    ...
    const req = reqFor("/members/x", { cookie: `__Host-edge_session=${value}` });
  ...
  it("returns null for a tampered session", async () => {
    ...
    expect(await readSession(reqFor("/m", { cookie: `__Host-edge_session=${tampered}` }), config)).toBeNull();
  });

  it("returns null instead of throwing for malformed percent-encoded cookie values", async () => {
    await expect(readSession(reqFor("/m", { cookie: "__Host-edge_session=%" }), config))
      .resolves.toBeNull();
  });

  it("returns null instead of throwing for invalid base64 session tokens", async () => {
    await expect(readSession(reqFor("/m", { cookie: "__Host-edge_session=not-valid!!!" }), config))
      .resolves.toBeNull();
  });
  ...
  it("returns null instead of throwing for malformed login-state cookies", async () => {
    await expect(readStateCookie(reqFor("/.auth/callback", { cookie: "__Host-edge_login=%" }), config))
      .resolves.toBeNull();
    await expect(readStateCookie(reqFor("/.auth/callback", { cookie: "__Host-edge_login=garbage!!!" }), config))
      .resolves.toBeNull();
  });
  ...
  it("state cookie round-trips and clears", async () => {
    ...
    expect(setCookie).toContain("__Host-edge_login=");
    const value = setCookie.match(/__Host-edge_login=([^;]*)/)[1];
    const saved = await readStateCookie(reqFor("/.auth/callback", { cookie: `__Host-edge_login=${value}` }), config);
